  ...
  function getShiftCodeForm ($type) {
    ...
    (function () use (&$form_shiftCode, $timestamp) {
      ...
      $props = [
        ...
        'inputProperties' => [
          'placeholder'      => $timestamp->format(DATE_FORMATS['date']),
          'type'             => 'date',
          'value'            => $timestamp->format(DATE_FORMATS['date']),
          'validations'      => [
            'required'          => true,
            'type'              => 'date',
            'validations'       => [
              'range' => [
                ...
                'min'              => $minExpiration->format(DATE_FORMATS['date']),
                'max'              => "\${expiration_date_value_date}|{$timestamp->format(DATE_FORMATS['date'])}"
              ]
            ]
            // 'required'          => true,
          ]
      ...
      (function () use (&$group, $timestamp) {
        ...
        $props = [
          ...
          'inputProperties' => [
            ...
            'value'            => [
              // 'date'              => $defaultExpiration->format(DATE_FORMATS['date']),
              'time'              => "23:59:59",
            ...
            'validations'      => [
              ...
              'validations'       => [
                'range' => [
                  'min'               => [
                    'date'               => "\${release_date}|{$timestamp->format(DATE_FORMATS['date'])}"
                  ],
                  'max'               => [
                    'date'               => $maxExpiration->format(DATE_FORMATS['date'])
                  ]
                ]
              ]
            ]
                ...
                'controls'  => [
                  'expiration_date_value_date' => [
                    'value'           => (function () use ($timestamp) {
                      ...
                      return $expiration->format(DATE_FORMATS['date']);
                    })()
                  ],
                  'expiration_date_value_time' => [
                    // 'readonly'        => false,
