...
?><!doctypehtml><html lang=en><meta charset=utf-8><?php include_once('global/sharedStyles.php'); ?><link href="/assets/css/local/help/faq.css<?php echo TK_VERSION_STR; ?>"rel=stylesheet><?php include_once('global/head.php'); ?><body data-theme=main><?php include_once('global/beforeContent.php'); ?><?php include_once('global/main-header.php'); ?><?php
    ...
    ?><main class=content-wrapper><header class="dropdown-panel toc"><button class="dropdown-panel-toggle header"><div class=wrapper><div class=title><div class=icon><span class="fas fa-list-alt"></span></div><div class=string><h2 class=primary>Table of Contents</h2></div></div><div class=indicator><span class="fas fa-chevron-right"></span></div></div></button><div class="body content-container"id=table_of_contents><?php foreach ($questions as $section => &$questionList) : ?><div class=section><h3><a href="<?= '#' .clean_url($section); ?>"><?= $section; ?></a></h3><ul class=styled><?php foreach ($questionList as &$question) : ?><?php $title = clean_html('Jump to question: "' . $question['q'] . '"'); ?><li><a href="<?= "#{$question['id']}"; ?>"aria-label="<?= $title; ?>"title="<?= $title; ?>"><?= $question['q']; ?></a></li><?php endforeach; ?></ul></div><?php endforeach; ?></div></header><?php foreach ($questions as $section => &$questionList) : ?><section class=faq-group><header id="<?= clean_url($section); ?>"><h2 class=title><?= $section; ?></h2></header><?php foreach ($questionList as &$question) : ?><?php $title = 'Jump to question: "' . $question['q'] . '"'; ?><div class="dropdown-panel c"id="<?= $question['id']; ?>"><h3 class=primary><?= $question['q']; ?></h3><div class=body><?= $question['a']; ?></div></div><?php endforeach; ?></section><?php endforeach; ?></main><?php include_once('local/support-footer.php'); ?><?php include_once('global/afterContent.php'); ?><?php include_once('global/sharedScripts.php'); ?>